...
from app.config import CHUNK_OVERLAP_WORDS, CHUNK_SIZE_WORDS, DATA_DIR
from app.logging_setup import get_logger, timed
from app.pdf_utils import chunk_document, extract_pages
from app.vectorstore import add_chunks, delete_source
...
def ingest_data_folder(force: bool = False, progress=None) -> List[Dict]:
    """
    Ingests every PDF in DATA_DIR that is new or has changed on disk. Safe to call
    repeatedly - unchanged files are reported, not re-embedded.
